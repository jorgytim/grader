...
function Get-Labresults {
	...
	$n1 =  Test-NsLookupHost -hostname $domain -points 2
	out-score "Resolving domain [$domain]?" $n1 2

	$n2 =  Test-PingHost -host $linux -points 1
	out-score "Pinging host [$linux]?" $n2 1

	$n3 =  Test-ComputerOnDomain -fqdn $winclient -points 1
    out-score "Computer on Domain [$winclient]?" $n3 1

	$n4 =  Test-UserHasLoggedOn -user $user1 -points 1
	out-score "Verify AD user exists and has logged on: [$user1]?" $n4 1

	$n5 = Test-UserHasLoggedOn -user "$user2" -points 1
	out-score "Verify AD user exists and has logged on: [$user2]?" $n5 1

    $n6 = Test-UserHomedirExists -user $user1 -points 1
    out-score "Homedir exists for [$user1]?" $n6 1

    $n7 = Test-UserHomedirExists -user $user2 -points 1
    out-score "Homedir exists for [$user2]?" $n7 1

    $n8 = Test-GroupExists -group $group1 -points 1
	out-score "Searching for AD group [$group1]?" $n8 1

    $n9 = TEst-MembersOfGroup -users $groupMbrs -group "$group1" -points 1
    out-score "Check membership for group [$group1]?" $n9 1
...
$labName = "IST346 Lab05 Check Script - Active Directory"
$labNum = "Lab05"
