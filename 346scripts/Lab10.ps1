...
function Grade-Lab {
    ...
    if (!($keyChk)) {
        write-host "Lab Prep script has not been run yet, this lab cannot be graded." -ForegroundColor Yellow
        exit
    }
    ...
    $n1 = Test-PingHost -hostname $linux -points 2
	out-score "Pinging host [$linux]?" $n1 2
	
    $n2 = Test-PingHost -hostname $winclient -points 2
	out-score "Pinging host [$winclient]?" $n2 2
    
    $n3 = Test-FileShare -computer $winsrv -share $share1 -points 1
	out-score "Checking if [$share1] is shared on [$winsrv]?" $n3 1
    
    $n4 = Test-FileShare -computer $winsrv -share $share2 -points 1
	out-score "Checking if [$share2] is shared on [$winsrv]?" $n4 1
    
    $n5 = Test-UserHomedir -user $user1 -points 2
    out-score "Homedir setting for [$user1]?" $n5 2

    $n6 = Test-UserHomedir -user $user2 -points 2
    out-score "Homedir setting for [$user2]?" $n6 2
...
$labName ="IST346 Lab10 Check Script - Troubleshooting"
$labNum = "Lab10"
