...
function Grade-Lab {
	...
	$share = "\\$($winsrv)\website"
	$group = "web-publishers"
    $groupmbmrs = "titupp","benweave","fenstein","adente"
	$web = "http://www.$($domain)"
	$www_dns = "www.$($domain)"
	$www_port = "80"
	$webmsg = "Welcome to Fauxco.com!"
	$blog_dns = "blog.$($domain)"
	$blog_port = "80"
	$blogmsg = "Welcome to WordPress"
	$blog = "http://blog.$($domain)/blog"
	$blog_testpost1 = "benweave"
	$blog_testpost2 = "titupp"
	$blog_testpost3 = "fenstein"
	$blog_testpost4 = "adente"
	...
	#check Win2008 website
	$n1 = Test-GroupExists -group $group -points 1
	out-score "Group exists [$group]?" $n1 1
	    
	$n2 = Test-MembersOfGroup -users $groupmbmrs -group $group -points 1
	out-score "Check membership for group [$group]?" $n2 1

	$n3 = Test-TCPPort -endpoint $www_dns -tcpport $www_port -points 1
	out-score "Is the Web service (port $www_port) runnning on [$www_dns]?" $n3 1
	
	$n4 = Test-Website -url $web -text $webmsg -points 1
	out-score "Does Site [$web] exist with title [$webmsg]?" $n4 1

	#check Linux website
	$n5 = Test-TCPPort -endpoint $blog_dns -tcpport $blog_port -points 1
	out-score "Is the httpd service port ($blog_port) runnning on [$blog_dns]?" $n5 1

	$n6 = Test-Website -url $blog -text $blogmsg -points 1
	out-score "Is the blog installed on Ubuntu? [$blog]" $n6 1
	    
	$n7 = Test-Website -url $blog -text $blog_testpost1 -points 1
	out-score "Did [$blog_testpost1] post a message that was approved on the blog? [$blog]" $n7 1
	
	$n8 = Test-Website -url $blog -text $blog_testpost2 -points 1
	out-score "Did [$blog_testpost2] post a message that was approved on the blog? [$blog]" $n8 1
	
	$n9 = Test-Website -url $blog -text $blog_testpost3 -points 1
	out-score "Did [$blog_testpost3] post a message that was approved on the blog? [$blog]" $n9 1
	
	$n10 = Test-Website -url $blog -text $blog_testpost4 -points 1
	out-score "Did [$blog_testpost4] post a message that was approved on the blog? [$blog]" $n10 1
...
$labName = "IST346 Lab09 Check Script - Web services"
$labNum = "Lab09"
