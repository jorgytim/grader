...
function Grade-Lab {
    ...
    $n2 = Test-TCPPort -endpoint $winsrv -tcpport 445 -points 1
	out-score "Checking availability of file sharing services on [$winsrv]?" $n2 1
    
    $n3 =  Test-FileShare -computer $linux -share $linshare -points 1
    out-score "Fileshare [$linshare] exists on [$linux]?" $n3 1
    
    $n4 = Test-FileShareWrite -unc $linshareUnc -user $usr -pass $upass -points 1
    out-score "Fileshare [$linshareUnc] writeable by [$usr]?" $n4 1
    
    $n5 =  Test-FileShare -computer $winsrv -share $winshare -points 1
    out-score "Fileshare [$winshare] exists on [$winsrv]?" $n5 1
    
    $n6 = Test-FileShareWrite -unc $winshareUnc -user $usr -pass $upass -points 1
    out-score "Fileshare [$winshareUnc] writeable by [$usr]?" $n6 1
    
    $n7 = Test-FileShare -computer $linux -share $linshare2 -points 1
    out-score "Fileshare [$linshare2] exists on [$linux]?" $n7 1
    
    $n8 = Test-FileShareWrite -unc $linshare2Unc -user $usr -pass $upass -points 1
    out-score "Fileshare [$linshare2Unc] writeable by [$usr]?" $n8 1
    
    $n9 = Test-FileShare -computer $winsrv -share $winshare2 -points 1
    out-score "Fileshare [$winshare2] exists on [$winsrv]?" $n9 1
    
    $n10 = Test-FileShareWrite -unc $winshare2Unc -user $usr -pass $upass -points 1
    out-score "Fileshare [$winshare2Unc] writeable by [$usr]?" $n10 1
...
$labName ="IST346 Lab03 Check Script - Servers"
$labNum = "Lab03"
